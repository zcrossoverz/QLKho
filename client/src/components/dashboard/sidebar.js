...
export default function Sidebar() {
    ...
    const listNav = [
        ...
        { icon: TruckIcon, path:"/khohang" },
        { icon: CurrencyDollarIcon, path:"/khohang" },
        { icon: OfficeBuildingIcon, path:"/khohang" },
        { icon: IdentificationIcon, path:"/khohang" },
        { icon: FilterIcon, path:"/khohang" },
        { icon: PlusCircleIcon, path:"/khohang" },
        { icon: AdjustmentsIcon, path:"/khohang" },
    ];
    ...
    let Button = (props) => {
        ...
        return (
                <div className={ props.active ? 'bg-gray-800 rounded-l-xl relative after:absolute after:w-4 after:h-8 after:-bottom-8 after:rounded-tr-xl after:right-0 after:shadow-inverse-bottom before:absolute before:w-4 before:h-8 before:-top-8 before:rounded-br-xl before:right-0 before:shadow-inverse-top' : ''}>
                    <button className='p-3 my-3 ml-3 mr-4 text-primary rounded-xl hover:bg-primary hover:text-white hover:shadow-primary'
                        onClick={() => {
  ...
  return (
    <div className='flex flex-col bg-gray-900 py-4 pl-2 items-center'>
        <div className='p-2 rounded-xl bg-opacity-20 bg-orange-500'>
